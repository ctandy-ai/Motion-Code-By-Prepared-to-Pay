...
- **Athlete Management**: Comprehensive profiles, team/position tracking, program assignment, and CSV import. Athlete detail page uses a **tabbed layout** with 5 tabs: Testing (default), Overview, Programs, Training, Wellness. Testing tab is the default/first tab for quick coach access to VALD data and normative benchmarks. Summary stat cards (Belt, Active Programs, VALD Tests, Workouts, Wellness Score) remain visible above tabs. Full inline editing: Edit Athlete dialog (name/email/phone/team/position/status/notes), Belt Override dropdown (click belt badge to change WHITE/BLUE/BLACK), Coach-side Wellness Check-in dialog (sliders for all metrics), Coach-side Workout Log dialog (exercise select, sets, reps, weights).
...
- **VALD Hub Integration**: Connects to the VALD Hub API for syncing athlete testing data (ForceDecks, NordBord, etc.), including OAuth 2.0 authentication, profile syncing, test data synchronization, display on athlete profiles, and AI Coach context integration. API URLs use service-specific patterns: `prd-{region}-api-externalprofile`, `prd-{region}-api-extforcedecks`, `prd-{region}-api-externaltenants`. Region codes: Australia=aue, Europe=euw, US=use. Profiles response wrapped in `{profiles:[...]}`, ForceDecks tests in `{tests:[...]}`. Field names: profileId, givenName, familyName, testId, recordedDateUtc, testType. **Sync auto-links VALD profiles to athletes by name matching** and cascades athlete IDs to existing tests. Test retrieval falls back through profile relationship when direct athleteId query returns empty. ForceDecks external API returns test metadata (type, date, weight) but not detailed result metrics - individual test/trials endpoints return 404. ForceDecks metric ID mapping (40 common metrics) included in `server/vald-hub.ts` for when API starts returning `extendedParameters`. Frontend shows full test type labels (CMJ→Countermovement Jump etc.) and body weight from test metadata.
- **Normative Data Reference System**: Research-backed benchmarks for comparing athlete VALD test results against population norms. Database tables: `normative_cohorts` (21 cohorts for CMJ, SJ, DJ, IMTP, NordBord across sex/age/sport groups) and `normative_metrics` (59 metrics with percentile bands p5-p95, z-score parameters, risk thresholds). Service (`server/normative-service.ts`) performs percentile calculation via interpolation or z-score CDF, cohort auto-matching by device/test/sex/sport/age with sport-specificity priority. API routes: `GET /api/normative/cohorts`, `GET /api/normative/cohorts/:id/metrics`, `POST /api/normative/compare`, `GET /api/vald/athletes/:id/norms`. Frontend component (`client/src/components/normative-comparison.tsx`) displays percentile bars with risk flags, cohort selector, and source citations on athlete detail pages. Athlete training profiles extended with sex, sport, training_level, dominant_limb, body_mass_kg, height_cm for cohort matching.
